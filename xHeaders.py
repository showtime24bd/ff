import os , psutil , sys , jwt , pickle , json , binascii , time , urllib3 , base64 , datetime , re ,socket , threading , ssl , asyncio , aiohttp , random
    ...
    while True:
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get('https://tokens-asfufvfshnfkhvbb.francecentral-01.azurewebsites.net/ReQuesT?&type=ToKens') as r:
                    t = await r.text()
                    i = t.find("ToKens : [")
                    if i != -1:
                        j = t.find("]", i)
...
async def Likes(id):
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(f"https://tokens-asfufvfshnfkhvbb.francecentral-01.azurewebsites.net/ReQuesT?id={id}&type=likes") as response:
...
async def Requests_SPam(id):
    async with aiohttp.ClientSession() as session:
        async with session.get(f'https://tokens-asfufvfshnfkhvbb.francecentral-01.azurewebsites.net/ReQuesT?id={id}&type=spam') as Api:
            if Api.status in [200, 201]:
                text = await Api.text()
                if '[SuccessFuLy] -> SenDinG Spam ReQuesTs !' in text:
                    return True
            return False
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, headers=headers, data=data, ssl=ssl_context) as response:
            if response.status == 200 or response.status == 201:
                ...
                packet = binascii.hexlify(content).decode('utf-8')
...
async def DeLet_Uid(id , Tok):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, headers=headers, data=data, ssl=ssl_context) as ResPonse:
            text = await ResPonse.text()
            if ResPonse.status == 400 and 'BR_FRIEND_NOT_SAME_REGION' in text:
...
async def ChEck_The_Uid(id):
    async with aiohttp.ClientSession() as session:
        async with session.get("https://panel-g2ccathtf6gdcmdw.polandcentral-01.azurewebsites.net/Uids") as Api:
            if Api.status not in [200, 201]: 
                return False    
            text = await Api.text()
